(() => {
  ...
  function loadDesign() {
    ...
    return normalizeDesign(saved || window.PUBLISHED_DESIGN || {});
  }
  ...
  function exportPublishedDesign() {
    ...
    const safeJson = JSON.stringify(design).replace(/</g,'\\u003c');
    const contents = `// Generated by Sprite Checklist. Replace this file in your GitHub repository.\nwindow.PUBLISHED_DESIGN = ${safeJson};\n`;
    const blob = new Blob([contents], { type:'text/javascript' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'published-design.js';
    link.click();
    setTimeout(() => URL.revokeObjectURL(url),0);
    showToast('Public design downloaded');
  }
